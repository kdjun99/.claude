...
### Criterion 4c: Problem Definition Question
- **Rule:** At least 1 question per document that probes the problem FRAMING process
- **How to check:** Search for questions that ask about:
  - Problem framing/definition ("문제를 어떻게 정의하셨나요?", "문제의 범위와 경계는?")
  - Problem validation ("정의한 문제가 실제 핵심 문제가 맞는지?")
  - Problem reframing ("처음 정의한 문제와 실제 문제가 달랐던 경험?")
  - Problem scope setting ("여러 문제 중 이 문제를 우선 해결하기로 한 근거?")
- **Pass:** ≥ 1 question found that probes problem framing process (not just "배경이 뭔가요?")
- **Fail:** No problem framing questions found. Generic background questions alone are INSUFFICIENT — they ask about context, not the process of defining and validating the problem.
- **Fail examples:** "이 기획의 배경이 무엇인가요?", "왜 이 기능을 만들게 됐나요?" — these ask for context, not problem definition process
- **Pass examples:** "문제를 어떻게 정의하셨나요?", "문제의 범위와 경계는 어떻게 설정했나요?", "처음 정의한 문제가 맞는지 어떻게 검증했나요?"
...
| 4c | Problem Definition Question | PASS/FAIL | Found problem definition question in X.Y / No problem framing question found |
