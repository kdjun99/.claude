...
# /dev:interview Command

Generate a structured 30-minute **developer** interview question document from candidate documents.

**Usage:** `/dev:interview <candidate_name> <resume_pdf_path> <portfolio_pdf_path>`
...
3. Create workspace: `~/.claude/workspace/dev-interview-question-generator/{candidate_name}/`
4. Create input directory: `{workspace}/input/`
5. If either PDF is missing, report error and STOP

## Step 1: Candidate Analysis

Delegate to `dev-candidate-analyzer` agent (opus):
...
workspace_path: ~/.claude/workspace/dev-interview-question-generator/{candidate_name}/
```
...
3. **Human Checkpoint:**
   ...
   - If feedback: note it and re-run dev-candidate-analyzer with feedback context
   - If proceed: continue to Step 2

## Step 2: Project Deep-Dive

Delegate to `dev-project-analyzer` agent (opus):
...
3. **Human Checkpoint:**
   ...
   - Options: "Proceed" / "I have feedback"
   - If feedback: note it and re-run dev-project-analyzer with feedback context
   - If proceed: continue to Step 3

## Step 3: Question Generation

Delegate to `dev-question-generator` agent (sonnet):
...
3. **Human Checkpoint:**
   ...
   - If feedback: note it and re-run dev-question-generator with feedback context
   - If proceed: continue to Step 4

## Step 4: Quality Verification

Delegate to `dev-quality-verifier` agent (haiku):
