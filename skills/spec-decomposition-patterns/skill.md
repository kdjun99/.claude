...
**Per-Repo Adjustment (reference multi-repo-structure Sizing Guidance):**

| Repo | Size Tendency | Adjustment |
|------|--------------|------------|
| linkareer-main | Medium-Large features | May need more aggressive splitting |
| linkareer-xen | Small-Medium | Standard sizing usually fits |
| linkareer-chat-api | Small | Rarely needs splitting |
| linkareer-point-api | Small, REST-only | Controller endpoints instead of resolvers |
| linkareer-cms-api | Small-Medium, REST-only | Controller endpoints, often read-only |
| linkareer-serverless | Per Lambda | 1 Lambda = 1 feature-request |
...
3. **Cross-repo dependencies use repo prefix** — `depends_on: [linkareer-main/01-feature-foundation]`
...
xen/01-feature-b.md           (depends_on: [linkareer-main/01-shared-foundation])
```
...
```
Group A (linkareer-main): Application Status
Group B (linkareer-main): Resume Scoring
Group C (linkareer-serverless): Status Notifications
...
| Repo identification | `Repo` field in Metadata maps to multi-repo-structure |
